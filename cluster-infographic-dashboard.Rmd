...
exemplars.frame <- read_csv("exemplar-communities.csv")
...
#exemplars.frame <- read_csv("example-communities.csv")
...
In identifying the ten community types, the following scales were used:
  ...
  * **Community Landscape Scale**: An indication of what the community "looks like," based
    on a combination of the US Census-designated Rural Urban Continuum Code (RUCC) and
    the algorithm-derived Urbanization Perceptions Small Area Index (UPSAI). The 
...
```{r}
streamgraph(months.frame, key = "community_type", value="n", date="date", interpolate = "step") %>%
...
```{r}
valueBox("Details", icon = "fas fa-info-circle", color = colors.iu[9],
         caption = "Details of the community types")
```

<!--Row
----------------------------------------------------------

### `r fa("fas fa-info-circle")` Exploring Community Types
...
```{r}
print(create.tile(ct))
```

### `r fa("fas fa-city")` Involvement in Big Cities

```{r}
print(create.tile_agents(ct))
```
...
```{r, results='hidden'}
ct <- "Big Cities"
photo.frame <- exemplars.frame %>% filter(community_type == ct)
photo.number <- sample(1:4, 1)
photo.label <- photo.frame$label[[photo.number]]
photo.file <- photo.frame$cityID[[photo.number]]
photo.ext <- photo.frame$img[[photo.number]]
photo.display <- glue("community-images/{photo.file}.{photo.ext}")
photo.link <- photo.frame$link[[photo.number]]
photo.attr <- photo.frame$attribution[[photo.number]]
display.photo <- glue("![Photo of {photo.label}]({photo.display})\n")
display.credit <- glue("Attribution: [{photo.attr}]({photo.link})")
